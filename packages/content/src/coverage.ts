...
export const MECHANIC_DETECTORS: Record<Mechanic, (character: CharacterDefinition, profile: CharacterProfile) => boolean> = {
  ...
  pets: (_c, p) => summonedIds(p).some((id) => /\b(pet|familiar|companion)\b/i.test(SUMMON_LIBRARY[id]?.displayName ?? "")),
  ...
  "water/tides": (_c, p) => mentions(p, /\b(tide|undertow|river|sea|wave|rain|downpour|water)\b/),
  ...
  ink: (_c, p) => p.resourceIds.has("resource.ink") || mentions(p, /\bink\b/),
